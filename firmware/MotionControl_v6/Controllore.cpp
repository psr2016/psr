...
Controllore::Controllore(I2C & IMU, float kp,float ki,float Saturazione)
...
void Controllore::ReadGyro()
{
    ...
    int out_x = IMU_comunication.read_buffer(0xD6, 0x28 | 0x80, &omega_x8[0], 2);   //data_len in byte
    int out_y = IMU_comunication.read_buffer(0xD6, 0x2A | 0x80, &omega_y8[0], 2);   //data_len in byte
    int out_z = IMU_comunication.read_buffer(0xD6, 0x2C | 0x80, &omega_z8[0], 2);   //data_len in byte
