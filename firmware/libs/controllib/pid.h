...
class Pid {
 public:
    ...
    float evaluate_using_error(float error, float target, float process_var);
    float derivative(float error, float process_var);
    float derivative_low_pass(float val);
    void set_derivative_low_pass(float cutoff);
 ...
 protected:
    ...
    float m_dt;	
    float m_saturation;
    LowPassFilter * m_derivative_low_pass;
};

class PidDerivateOutput : public Pid {
 public:
    float derivative(float error, float process_var);
};
